...
contextBridge.exposeInMainWorld("ipcRenderer", {
  ...
  newTab: (callback: any) =>
    ipcRenderer.on("new-tab", (_, data) => callback(data)),

  aiContextMenuSelectText: (callback: any) =>
    ipcRenderer.on("ai-context-menu-selecttext", (_, data) => callback(data)),
  ...
  getAppPath: (callback: any) => {
    ipcRenderer.on("message-to-parent-app", (_, message) => callback(message)); // Send to parent window
  },
  ...
  getAppPathMain: (message: any) => {
    ipcRenderer.send("getAppPathMain", message); // Send to main process
  },
});
